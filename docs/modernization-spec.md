...
| Storage commit | idle; writing; replace succeeded/verifying; committed; uncommitted; indeterminate/recovery | Publish no Recent Code or effect while writing/verifying; resolve by authenticated revision/checksum read-back; stop intake and offer recovery if unresolved |
...
- **FR-14:** Acceptance proposes one update of a single versioned encrypted state snapshot: add the Seen Message identity, add the History entry when retention is not Off, and add eligible payload-free Auto-copy/notification intent metadata. Before atomic replace succeeds, any failure is uncommitted. After replace, a parent-directory sync failure is indeterminate, never rollback. No Recent Code or effect is published until authoritative read-back validates authenticated content and compares the expected revision/checksum, resolving the current file as the new committed snapshot or the prior snapshot. An unreadable or unresolved result enters storage recovery with intake/effects stopped.
...
An intake acceptance proposes one encrypted snapshot replacement. It adds the Seen Message identity, adds a History entry when retention is not Off, and adds payload-free intent metadata for each eligible notification/Auto-copy attempt. A failure before atomic replace is uncommitted. After replace, parent-directory sync failure is indeterminate. In either successful or indeterminate replace, authoritative read-back must authenticate and match the expected new or prior revision/checksum before deciding the commit; unreadable/unresolved storage enters recovery. Only a resolved new commit permits the in-memory Detected OTP to enter Recent Codes or effects.
...
| Atomic replace succeeds and parent-directory sync succeeds | Commit still awaits authoritative read-back | If authenticated read-back matches expected new revision/checksum, commit and publish; if it matches prior, treat uncommitted and retry; otherwise enter recovery |
| Atomic replace succeeds and parent-directory sync errors | Commit is indeterminate, never assumed rolled back | Read back authoritatively: matching new commits; matching prior is uncommitted; unreadable/missing/mismatched state enters recovery; no UI/effect until resolved |
...
Fault-injection tests must stop the process at every row and specifically cover replace success plus parent-sync error with read-back returning old, new, and unreadable content. They reopen the store and prove snapshot integrity, Seen/History agreement, Recent Code restart semantics, payload-free durability, no post-restart effect replay, and no post-deletion related effect.
...
| Integration | every section 9.4 crash boundary including replace-success/parent-sync-error old/new/unreadable read-back; migration/restart/key loss; disconnect/restart; repeated unread Message; no effect replay; clear/delete cancellation; notification permission; Start at Login request/read-back/persist/restart/drift; signed update; command capability allow/deny |
...
- every section 9.4 crash boundary resolves commit state by authenticated revision/checksum read-back, and replace-success/parent-sync-error tests cover old, new, and unreadable results;
...
| Refinement — atomic commit resolution | FR-09, FR-14, section 9.4 | pre-replace uncommitted plus post-replace sync-error old/new/unreadable read-back | M1–M2 / tasks 3, 20 |
